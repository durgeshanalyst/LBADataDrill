...
const app = {
    ...
    async init() {
        this.editor = CodeMirror.fromTextArea(document.getElementById('code-editor'), {
            lineNumbers: true, theme: 'dracula', tabSize: 4, indentUnit: 4, lineWrapping: true
        });
    ...
    renderTopicFilters() {
        ...
        topics.forEach(topic => {
            ...
            btn.className = `topic-tag px-2 py-1 text-[10px] border border-gray-700 rounded bg-gray-800 text-gray-400 hover:border-blue-500 hover:text-white transition mb-1 mr-1`;
            btn.innerText = topic;
            btn.onclick = () => {
    ...
    renderSidebar(problems) {
        ...
        problems.forEach(p => {
            ...
            item.id = `prob-item-${p.id}`; // Give ID for selection update
            
            // Check if this is the active problem
            ...
            const inactiveStyles = 'border-transparent text-gray-400 hover:bg-gray-800 hover:border-yellow-500 hover:text-white';
            const activeStyles = 'bg-gray-800 border-yellow-500 text-white'; // Active look
            
            item.className = `${baseStyles} ${isActive ? activeStyles : inactiveStyles}`;
            
            let diffColor = 'bg-gray-500';
            ...
            item.innerHTML = `
                ...
                <div class="text-[10px] text-gray-500 mt-1">${p.topic || 'General'}</div>
            `;
    ...
    loadProblem(id) {
        ...
        // Update sidebar UI to show selection
        this.updateSidebarSelection(id);
        
        let htmlContent = `
            <h2 class="text-2xl font-bold mb-2 text-white">${problem.title}</h2>
            <div class="flex gap-2 mb-4">
                <span class="px-2 py-1 rounded bg-gray-700 text-xs text-gray-300">${problem.difficulty}</span>
                <span class="px-2 py-1 rounded bg-blue-900/50 text-xs text-blue-300 border border-blue-800">${problem.topic || 'General'}</span>
            </div>
            <div class="mt-4 text-gray-300 leading-relaxed">${problem.description}</div>
        `;
    ...
    // NEW: Helper to update visual selection
    updateSidebarSelection(activeId) {
        // Reset all items
        document.querySelectorAll('#problem-list > div').forEach(el => {
            el.className = 'p-3 cursor-pointer border-l-2 transition text-sm mb-1 border-transparent text-gray-400 hover:bg-gray-800 hover:border-yellow-500 hover:text-white';
        });
        
        // Highlight active item
        const activeItem = document.getElementById(`prob-item-${activeId}`);
        if(activeItem) {
            activeItem.className = 'p-3 cursor-pointer border-l-2 transition text-sm mb-1 bg-gray-800 border-yellow-500 text-white';
        }
    }
};
...
const schemaUtils = {
    generateHTML(sql) {
        ...
        let match;
        let html = '<div class="mt-8 border-t border-gray-700 pt-4"><h3 class="text-sm font-bold text-gray-400 uppercase mb-4">Database Schema</h3>';
        ...
        while ((match = tableRegex.exec(sql)) !== null) {
            ...
            html += `
                <div class="mb-4 border-2 border-dashed border-gray-700 rounded-lg bg-gray-900/50 p-4">
                    <div class="flex items-center gap-2 mb-3 border-b border-gray-700 pb-2">
                        <i class="fa-solid fa-table text-blue-400"></i>
                        <span class="font-bold text-white">${tableName}</span>
                    </div>
                    <div class="grid grid-cols-1 gap-y-1">
                        ${columns.map(col => `
                            <div class="flex justify-between text-xs font-mono">
                                <span class="text-gray-300">${col.name}</span>
                                <span class="text-gray-500 uppercase">${col.type}</span>
                            </div>
                        `).join('')}
                    </div>
                </div>
...
const authManager = {
    login() {
        if (app.state.mode === 'firebase') {
            ...
            const mockUser = { uid: 'local-user-1', displayName: 'Demo Student', photoURL: 'https://via.placeholder.com/40' };
    ...
    async loadProfileData() {
        ...
        document.getElementById('profile-view-name').innerText = app.state.user.displayName;
        document.getElementById('profile-view-avatar').src = app.state.user.photoURL;
        ...
        if(history.length === 0) {
            ...
            history.sort((a,b) => b.timestamp - a.timestamp).forEach(h => {
                ...
                tr.innerHTML = `
                    <td class="px-6 py-4 font-medium text-white">${h.problemTitle}</td>
                    <td class="px-6 py-4"><span class="px-2 py-1 rounded text-xs bg-gray-700 uppercase">${h.track}</span></td>
                    <td class="px-6 py-4">
                        ${h.status === 'Passed' ? '<span class="text-green-400"><i class="fa-solid fa-check"></i> Passed</span>' : '<span class="text-red-400"><i class="fa-solid fa-xmark"></i> Failed</span>'}
                    </td>
...
const runner = {
    log(msg, isError = false, customClass = '') {
        ...
        if (div) {
            let colorClass = isError ? 'text-red-400' : 'text-gray-300';
            if (customClass) colorClass = customClass;
            div.innerHTML += `<div class="${colorClass} border-b border-gray-800 py-1 font-mono text-xs">> ${msg}</div>`;
    ...
    logTable(data) {
        ...
        let html = `
            <div class="overflow-x-auto my-2 border border-gray-700 rounded">
                <table class="min-w-full text-xs text-left text-gray-300">
                    <thead class="bg-gray-800 text-gray-400 uppercase font-medium">
                        <tr>${headers.map(h => `<th class="px-4 py-2 border-b border-gray-700">${h}</th>`).join('')}</tr>
                    </thead>
                    <tbody class="divide-y divide-gray-800">
                        ${slicedData.map(row => `
                            <tr class="hover:bg-gray-800/50">
                                ${headers.map(h => `<td class="px-4 py-2 whitespace-nowrap font-mono">${row[h] !== null ? row[h] : '<span class="text-gray-600">NULL</span>'}</td>`).join('')}
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
    ...
    async runPython(code, problem, isSubmission) {
        ...
        try {
            ...
            if (!isSubmission) {
                if (result !== undefined) {
                    this.log("Result: " + result, false, "text-blue-400 font-bold");
                } else if (!outputCaptured) {
                    this.log("ℹ️ Code ran successfully.", false, "text-yellow-500");
                    this.log("   (Tip: Use print() to see output)", false, "text-yellow-600 italic");
                }
            }
            if (isSubmission) {
                this.log("\n--- Verifying Solution ---");
    ...
    runSQL(code, problem, isSubmission) {
        ...
        try {
            ...
            if(Array.isArray(userResult)) {
                ...
                else { this.log("Query Result:", false, "text-blue-400"); this.logTable(userResult); }
            } else {
